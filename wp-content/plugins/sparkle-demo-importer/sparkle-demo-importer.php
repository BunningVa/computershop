...
define('SPARKLE_DEMOI_VERSION', '1.0.2');
                        ...
                        if( is_array( $categories) && !empty( $categories )){
                            foreach ( $categories as $demo_index => $demo_cats ){
                                foreach ( $demo_cats as $cat_index => $single_cat ){
                                    ...
                                    ?>
                                    <li class="sparkle-theme-tab" data-filter=".<?php echo strtolower( esc_attr($single_cat) );?>">
        ...
        function importing_revslider() {
            ...
            if (file_exists($sliderFile)) {
                if (class_exists('RevSlider')) {
                    ...
                    $this->ajax_response['complete_message'] = esc_html__('Revolution slider installed', 'sparkle-demo-importer');
                }else{
                    $this->ajax_response['complete_message'] = esc_html__('Revolution slider plugin not installed', 'sparkle-demo-importer');
                }
            } else {
                $this->ajax_response['complete_message'] = esc_html__('No Revolution slider found', 'sparkle-demo-importer');
            }
